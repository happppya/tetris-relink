...
describe('Lobby', () => {
  ...
  it('only lets the host change settings and sanitizes payloads', () => {
    ...
    expect(lobby.settings).toEqual({ mode: 'winByX', goal: 99, winBy: 1, fourWide: false })
    expect(lobby.setSettings('host', { mode: 'firstToX', goal: 5, winBy: 2, fourWide: true })).toBe(true)
    expect(lobby.settings.fourWide).toBe(true)
