        ...
        match msg {
            ...
            FrontendMessage::Suggest => {
                if let Some((moves, move_info)) = bot.suggest() {
                    let moves = moves.into_iter().map(|(mv, _)| mv).collect();
                    outgoing
...
fn spawn_workers(bot: &Arc<BotSyncronizer>) {
    // background OS threads are not available on wasm32; the wasm wrapper
    // drives search cooperatively via BotSyncronizer::pump_work instead
