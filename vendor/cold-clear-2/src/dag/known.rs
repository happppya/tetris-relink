...
impl<'bump, E: Evaluation> Layer<'bump, E> {
    ...
    pub fn suggest(&self, state: &GameState) -> Vec<(Placement, f64)> {
        ...
        // NOTE(vendored): upstream extends with `children.first()` (a single
        // move); expose the full ranked list so embedders can apply their own
        // reachability constraints.
        candidates.extend(children.iter());
        candidates.sort_by(|a, b| a.cached_eval.partial_cmp(&b.cached_eval).unwrap().reverse());

        candidates.into_iter().map(|c| (c.mv, c.cached_eval.score())).collect()
    }
