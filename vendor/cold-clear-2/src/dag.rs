    ...
    Ord + Copy + Default + std::ops::Add<Self::Reward, Output = Self> + 'static
{
    ...
    /// numeric value for cross-search comparison by embedders (hold planning)
    fn score(self) -> f64;
}
...
impl<E: Evaluation> Dag<E> {
    ...
    pub fn suggest(&self) -> Vec<(Placement, f64)> {
...
impl<E: Evaluation> WithBump<E> {
    ...
    fn suggest(&self, state: &GameState) -> Vec<(Placement, f64)> {
        self.with(|this| match this.data {
